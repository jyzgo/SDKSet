...
public class StatisticsMgr : MonoBehaviour {
    ...
    int[] FastestWin = new int[2] { 99999, 99999 };
    int[] FewestMoves = new int[2] { 99999, 99999 };
    ...
    public void ShowWindow()
    {
        ...
        int fast = FastestWin[draw];
        if (fast == 99999)
        {
            fast = 0;
        }

        int fewest = FewestMoves[draw];
        if (fewest == 99999)
        {
            fewest = 0;
        }
        FastWinText.text =  fast.ToString();
        FewestMovesText.text = fewest.ToString();
        HighScoreText.text = HighScore[draw].ToString();
        Window.SetActive(true);
    }

    public SlideIn _staWindowSlide;
    public void HideWindow()
    {
        var t = _staWindowSlide.SlidOut();
        StartCoroutine(ActiveFalse(t, Window));
        
    }

    IEnumerator ActiveFalse(float t, GameObject g)
    {
