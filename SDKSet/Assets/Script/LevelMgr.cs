...
public class LevelMgr : MonoBehaviour
{
   ...
   public List<CardAction> _CardActions = new List<CardAction>();
    ...
    void OnApplicationQuit()
    {
        SaveGame();
        SettingMgr.current.SaveToFile();
        MTTracker.Instance.Track(SoliTrack.background, StatisticsMgr.current.WinsCount(), (Time.time - startTime).ToString());
    }
    ...
    {
        if (SettingMgr.current.GroupId % 2 == 0)
        {
            path = "mlv/mLevel"; 
        }
        lvid = n;
   ...
   // int lastPlayedLv = 0;
    ...
    public void ResetGame()
    {
        ...
        SettingMgr.current._adState.OnResetGame();
    ...
    const float offi = 0.08f;
    const float offj = 0.01f;
    void DoPlayDeal()
    {
        ...
        for (int i = 0; i < CardPlatform.Length; i++)
        {
            ...
            for (int j = 0; j < curCardList.Count; j++)
            {
                ...
                if (j == curCardList.Count - 1)
                {
                    seq = new MTSequence(
                        new MTDelayTime(1f),
                        new MTPlace(curCard.transform.localPosition + Vector3.back),
                        new MTMoveToWorld(i * offi + j * offj, tarPos),
                        new MTRotateTo(0.2f, new Vector3(0, 0, 0)),
                        new MTCallFunc(() => curCard.transform.eulerAngles = Vector3.zero));
                }
                else
                {
                    seq = new MTSequence(new MTDelayTime(1f), new MTPlace(curCard.transform.localPosition + Vector3.back), new MTMoveToWorld(i * offi + j * offj, tarPos));
                }
    ...
    IEnumerator SetPile(int index)
    {
        ...
        SettingMgr.current._adState.OnDealAnimDone();
    }
    ...
    IEnumerator Playing_Enter()
    {
        ...
        ToolbarMgr.current.SwitchToolbar(false);
        StartGame();
    ...
    IEnumerator Win_Enter()
    {
       ...
       // ToolbarMgr.current.ShowToolBar(false);
        ChallengeMgr.current.SetWin();
        isPlayingSuccess = true;
        PlayWinAnim();
        SoundManager.Current.PlayEndShuffle();
        AutoCollectBtn.SetActive(false);
        yield return new WaitForSeconds(ANIM_PLAYTIME);
        SettingMgr.current._adState.OnGameAnimDone();
        //Toolbar.SetActive(true);
    ...
    void PlayAnim1()
    {
        ...
        for (int i = 0; i < CardList.Count; i++)
        {
            ...
            curCard.RunActions(
                ...
                new MTRotateTo(0.3f, Quaternion.Euler(new Vector3(0,180,0))),
                 new MTMoveToWorld(0.3f, start.transform.position)
