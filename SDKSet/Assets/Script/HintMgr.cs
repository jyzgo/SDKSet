...
public class HintMgr : MonoBehaviour {
    ...
    internal void SetCardBack(Sprite sp)
    {
        var unused = _unusedCardSet.GetEnumerator();
        while (unused.MoveNext())
        {
            unused.Current.UpdateBack(sp);
        }

        var used = _usedCardSet.GetEnumerator();
        while (used.MoveNext())
        {
            used.Current.UpdateBack(sp);
        }
        //HashSet<Card> _usedCardSet = new HashSet<Card>();
    }
